...
static void *cpu_discharge_thread(void *arg)
{
	int i;
	double seed = 1.01d;
	printf("INFO:[%ul] cpu_discharge_thread created\n", *((int *)arg));
	while(1) {
		double sum = 0;
		...
		if (!discharge_status.cpu_discharge_enable) {
			printf("INFO:[%u] cpu_discharge_thread\n", *((int *)arg));
	...
	while (1) {
		if (discharge_status.flash_discharge_enable) {
			if (system("echo 510 > /sys/class/leds/torch-light0/brightness") != 0)
				printf("ERROR: flash_discharge_thread fail!\n");
			...
			if (system("echo 0 > /sys/class/leds/torch-light1/brightness") != 0)
				printf("ERROR: flash_discharge_thread fail!\n");
			printf("INFO: flash_discharge_thread exit\n");
	...
	while (1) {
		if (discharge_status.vibrate_discharge_enable) {
			if (system("echo 10000 > /sys/class/timed_output/vibrator/enable") != 0)
				printf("ERROR: vibrate_discharge_thread fail!\n");
		} else {
			printf("INFO: vibrate_discharge_thread exit\n");
...
void discharge_start(enum discharge_method method)
{
	...
	if (method & DISCHARGE_METHOD_CPU) {
		if (!discharge_status.cpu_discharge_enable) {
			printf("INFO: cpu discharge method start\n");
			...
			for (i = 0; i < CPU_PTHREAD_MAX; i++) {
				...
				if (err != 0) {
					printf("ERROR:[%d] cpu_discharge_thread create error!\n", i);
	...
	if (method & DISCHARGE_METHOD_FLASH) {
		if (!discharge_status.flash_discharge_enable) {
			printf("INFO: flash discharge method start\n");
			...
			if (err != 0) {
				printf("ERROR: flash_discharge_thread create error!\n");
	...
	if (method & DISCHARGE_METHOD_VIBRATE) {
		if (!discharge_status.vibrate_discharge_enable) {
			printf("INFO: vibrate discharge method start\n");
			...
			if (err != 0) {
				printf("ERROR: vibrate_discharge_thread create error!\n");
...
void discharge_stop(void)
{
	...
	discharge_status.vibrate_discharge_enable = 0;
	do {
		printf("INFO: waiting for discharge pthread stop...\n");
		sleep(2);
